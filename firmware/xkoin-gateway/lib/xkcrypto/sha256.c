// Compact SHA-256 (FIPS 180-4). Verified in test_core.c against the "abc"
// vector and against Python hashlib via node-id cross checks.
#include "sha256.h"
#include <string.h>
...
void xk_sha256(const uint8_t *data, size_t len, uint8_t out[32]) {
    ...
    for (int b = 0; b < 8; b++) tail[padded - 1 - b] = (uint8_t)(bits >> (8 * b));
    compress(h, tail);
