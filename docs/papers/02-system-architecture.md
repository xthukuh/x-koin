...
**The chain.** Three contracts on Base: `xKoinToken`, an ERC-20 with 6 decimals and EIP-2612 permit, where one base unit is a micro-KES; `xKoinEscrow`, which holds one deposit per user address and settles ticket batches; and `xKoinTreasury`, which receives the protocol fee and can pay only its beneficiary. Base was chosen because a settlement costs a fraction of a cent and there is nothing to operate; an own chain would cost more to run than the network earns. Status: built, 35 Foundry tests including a 256-run solvency fuzz and a stolen-owner-key drill.
